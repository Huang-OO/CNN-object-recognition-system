...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter("logs/log3/train", sess.graph)
        test_writer = tf.summary.FileWriter("logs/log3/test")
        batch_size = 50
        tf.global_variables_initializer().run()
        train_batch = train_next_batch(batch_size)
        test_batch = train_next_batch(batch_size)
        for i in range(2001):
            sess.run(train, feed_dict={x: train_batch[0], y: train_batch[1], keep_prob: 0.5})
            if (i % 100 == 0):
                summary = sess.run(merged, feed_dict={x: train_batch[0], y: train_batch[1], keep_prob: 0.5})
                train_writer.add_summary(summary, i)

                summary = sess.run(merged, feed_dict={x: test_batch[0], y: test_batch[1], keep_prob: 0.5})
                test_writer.add_summary(summary, i)
                saver_path = saver.save(sess, "Model/saved_cnn.ckpt")
                print("save path %s" % saver_path)

                train_accuracy = sess.run(accuracy, feed_dict={x: train_batch[0], y: train_batch[1], keep_prob: 0.5})
                print("after %d steps the trainnig accuracy is %g" % (i, train_accuracy))

                test_accuracy = sess.run(accuracy, feed_dict={x: test_batch[0], y: test_batch[1], keep_prob: 0.5})
